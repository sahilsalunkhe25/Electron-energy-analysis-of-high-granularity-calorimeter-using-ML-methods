...
file_path = "/content/Electron_latest.pkl"
...
train_data = data_list[0:7]
test_data = data_list[7:10]

# train_data[i]['y']=energy[i]
# test_data[i]['y']=energy[((4*num_test_graph)/5)+i]

# for i in range(0,int((4*num_test_graph)/5)):
#   # train_data[i]['y']=energy[i]
#   print("Training element",i,"is ",train_data[i],train_data[i]['y'])
# for i in range(0,int((1*num_test_graph)/5)):
#   # test_data[i]['y']=energy[int((4*num_test_graph)/5)+i]
...
for epoch in range(epochs):
    ...
    for batch in train_loader:
        ...
        target = batch.y.float()  # Convert target values to float
    ...
    if(mse<min_mse):
      ...
      torch.save(model,'/content/WeightElectronFile.pt')
...
print("Average Training Time per Epoch: {:.3f} seconds".format(avg_train_time))
print("Average Testing Time per Epoch: {:.3f} seconds".format(avg_test_time))
